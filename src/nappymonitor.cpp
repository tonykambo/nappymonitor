...
// Synology Mosquitto configuration
char server[] = IOT_URL;

char authMethod[] = "use-token-auth";
char token[] = IOT_TOKEN;

// original Watson IoT client ID
//char clientId[] = "d:" IOT_ORG ":" IOT_DEVICE_TYPE ":" IOT_DEVICE_ID;

char clientId[] = "nappymonitor:1001";
...
void connectWithBroker() {
  ...
  if (!!!client.connected()) {
    ...
    // Previously used to connect with Watson IoT
...
void sendSensorData() {
  ...
  if (!isnan(nappyTempC) && !isnan(nappyHumid)) {
      ...
      if (client.publish(sensorTopic, (char*) payload.c_str())) {
        Serial.print("Publish ok with client state:");
        Serial.println(client.state());
      } else {
        Serial.print("Publish failed with error:");
        Serial.println(client.state());
...
void processJson(char * message) {
  // ArduinoJson 5 -- StaticJsonBuffer<BUFFER_SIZE> jsonBuffer;
  ...
  // ArduinoJson 5 -- JsonObject& root = jsonBuffer.parseObject(message);

  //ArduinoJson 6
  //JsonObject root = deserializeJson(jsonDocument, message);
